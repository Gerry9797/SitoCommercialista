...
public class AutoFont {
   ...
   * NOTE: Should not be used repeatedly as each font found is parsed to get the
   * family name.
   *
   * @param validFileExtensions list of file extensions that are fonts - usually
   *                            Collections.singletonList("ttf")
   * @param recurse             whether to look in sub-directories recursively
   * @param followLinks         whether to follow symbolic links in the file
   *                            system
  ...
  public static List<CSSFont> findFontsInDirectory(
    ...
    Set<FileVisitOption> options = followLinks ? EnumSet.of(FileVisitOption.FOLLOW_LINKS)
  ...
  /**
   * Returns a list of fonts in a given directory. Recursively searches directory
   * and
  ...
  /**
   * Get a string containing added font families (duplicates removed) in a format
   * suitable
  ...
  public static class CSSFont {
    ...
    /**
     * WARNING: Heuristics are used to determine if a font is bold (700) or normal
     * (400) weight.
     */
    public final int weight;

    /**
     * WARNING: Heuristics are used to determine if a font is italic or normal
     * style.
     */
  ...
  public static class FontFileProcessor extends SimpleFileVisitor<Path> {
    ...
    protected void onValidFont(CSSFont font) {
      System.out.format("Adding font with path = '%s', name = '%s', weight = %d, style = %s%n", font.path, font.family,
          font.weight, font.style.name());
    }
